...
Word mulWW(Word x, Word y, Word* z1) {
	return UIntMul64(x, y, z1);
}

static Word mulAddWWW(Word x, Word y, Word c, Word* z1) {
	uint64_t hi = 0;
	uint64_t lo = UIntMul64(x, y, &hi);
	uint64_t cc = 0;
	lo = UIntAdd64(lo, c, &cc);
	*z1 = hi + cc;
	return lo;
}
...
static Word addMulVVW(nat* z, nat* x, Word y) {
	...
	for (uint64_t i = 0; i < z->len && i < x->len; ++i) {
		Word z1 = 0;
		Word z0 = mulAddWWW(x->nat[i], y, z->nat[i], &z1);
		Word lo = UIntAdd64(z0, c, &c);
		z->nat[i] = lo;
		c += z1;
	}
	return c;
...
static void basicMul(nat* z, nat* x, nat* y) {
	for (uint64_t i = 0; i < y->len; ++i) {
		if (y->nat[i] != 0) {
			nat* zz = natPart(z, i, i + x->len);
			z->nat[x->len + i] = addMulVVW(zz, x, y->nat[i]);
			natFree(zz);
		}
	}
}
...
Word mulAddVWW(nat* z, nat* x, Word y, Word r) {
	Word c = r;
	for (uint64_t i = 0; i < z->len && i < x->len; ++i) {
		z->nat[i] = mulAddWWW(x->nat[i], y, c, &c);
	}
	return c;
}
...
static void karatsuba(nat* z, nat* x, nat* y) {
	...
	memcpy(r->nat, &z->nat[0], n * 2 * sizeof(Word));
...
nat* natMul(nat* x, nat* y) {
	...
	if (n == 1) {
		return natMulAddWW(x, y->nat[0], 0);
	}
	...
	memset(z2->nat, 0, z2->len * sizeof(z2->nat[0]));
	natFree(z2);
