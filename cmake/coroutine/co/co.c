...
#ifdef _WIN32
  // 创建纤程
  co->ctx = CreateFiber(stack_size, co_main, env);
#else
  co->stack = NULL;
#endif
  return co;
  ...
  switch (status) {
    ...
    case CO_SUSPEND:
      ...
      if (co->ctx != NULL)
        SwitchToFiber(co->ctx);
#else
      memcpy(env->stack + STACK_SIZE - co->size, co->stack, co->size);
      swapcontext(&env->main, &co->ctx);
#endif
      return true;
    case CO_STOPED:
...
void co_yield (co_env* env) {
  ...
  _save_stack(co, env->stack + STACK_SIZE);
